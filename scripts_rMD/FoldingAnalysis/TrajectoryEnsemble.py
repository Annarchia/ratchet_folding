...
class TrajectoryEnsemble:
    ...
    def getAveragePenalty(self, folded_only=True):
        ...
        try:
            pen_dim = len(working_trajectories[0].getBiasProperties().bias_penalty)
            
            if not all([len(traj.getBiasProperties().bias_penalty) == pen_dim for traj in working_trajectories]):
        ...
        i=0
        for traj in working_trajectories:
            average_penalty[i] = traj.getBiasProperties().bias_penalty
            i+=1
    ...
    def DRP(self):
        ...
        return self.getFoldedTrajectories()[np.argmin([traj.getPenaltyAtFolding() for traj in \
                                                       self.getFoldedTrajectories()])]
    def DRPNormal(self):
        if not self.getFoldedTrajectoriesCount() > 0:
            return None
        
        return self.getFoldedTrajectories()[np.argmin([traj.getPenaltyNormalAtFolding() for traj in \
                                                       self.getFoldedTrajectories()])]
    def DRPComplete(self):
        if not self.getFoldedTrajectoriesCount() > 0:
            return None
        max_ftime = np.max([traj.getFoldingTime() for traj in self.getFoldedTrajectories()])
        return self.getFoldedTrajectories()[np.argmin([traj.getPenaltyAtTime(max_ftime) for traj in \
                                                       self.getFoldedTrajectories()])]
    ...
    def summary(self):
        ...
        for traj in sorted(self.getFoldedTrajectories(),key=lambda x: x.getFoldingTime()):
            ...
            try:
                summary += ' | Bias: ' + str(traj.getPenaltyAtFolding()) + ' | Complete Bias: '+ \
                       str(traj.getPenaltyAtTime(self.maxFoldingTime())) + ' | Normalised Bias: '+ \
                       str(traj.getPenaltyNormalAtFolding()) + '\n'
            except:
                summary += '\n'
        
        summary += '> Max folding time: ' + str(self.maxFoldingTime()) + '\n'
        summary += '> Min folding time: ' + str(self.minFoldingTime()) + '\n'
        summary += '> Mean folding time: ' + str(self.meanFoldingTime()) + '\n'
        summary += '> Median folding time: ' + str(self.medianFoldingTime()) + '\n'
        summary += '> Std folding time: ' + str(self.stdFoldingTime()) + '\n\n'
        try:
            summary += '> DRP is ' + self.DRP().getName() + ' (' + str(self.DRP().getPenaltyAtFolding()) + ')\n'
            summary += '> DRP (complete bias) is ' + self.DRPComplete().getName() + \
                       ' (' + str(self.DRPComplete().getPenaltyAtTime(self.maxFoldingTime())) + ')\n'
            summary += '> DRP (normalised bias) is ' + self.DRPNormal().getName() + \
                       ' (' + str(self.DRPNormal().getPenaltyNormalAtFolding()) + ')\n'
        except:
