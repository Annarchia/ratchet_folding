...
def hardQ(trajectory, ref, use_ref_as_last = False, min_dist = 3, cutoff = 7.5, selection = 'not type H'):
...
def bestHummerQ(trajectory, ref, use_ref_as_last = False, min_dist = 3, cutoff = 4.5, beta_c = 5, lambda_c = 1.8, selection='not type H'):
...
def ratchet_outParser(filename):
    ...
    return BiasProperties(time=ratchet_out[:,0],progress=ratchet_out[:,1],closeness=ratchet_out[:,2],
                          bias_penalty=ratchet_out[:,4], cum_tot_force=ratchet_out[:,5])
