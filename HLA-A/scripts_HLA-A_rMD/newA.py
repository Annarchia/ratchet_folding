...
if(len(sys.argv) < 3):
    print('ERROR!')
    print('Usage:python script.py woking_dir reference')
    sys.exit(1)

w_dir = sys.argv[1].rstrip('/')
...
#         print('ERROR: Cannot delete old result directory!')
#         raise
...
reference = fa.Trajectory(ref_pdb_name, name='reference')
...
try:
    ...
    for f in os.listdir(w_dir):
        ...
        if not os.path.isfile(i_traj):
            continue
        trajectories.append(fa.Trajectory(ref_pdb_name, trajectory_file=i_traj, name=f))
    ...
    print('> Folding Trajectories: '+str(ensemble.getFoldedTrajectoriesCount()))
    #mean_folding_time = ensemble.meanFoldingTime()
    #print('> Average folding time: '+str(mean_folding_time))
    ...
    average_cmaps = ensemble.getAverageCmapTime()
    ...
    c_q = utilities.cmap2hard_q(average_cmaps, reference.getCmap(cache=True)[0])
